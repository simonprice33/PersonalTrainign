...
```bash
# Get admin token
TOKEN=$(curl -s -X POST "https://subman-content.preview.emergentagent.com/api/admin/login" \
  -H "Content-Type: application/json" \
  -d '{"email":"simon.price@simonprice-pt.co.uk","password":"Qwerty1234!!!"}' \
  | python3 -c "import sys,json;print(json.load(sys.stdin)['accessToken'])")

# Try to update status
curl -X PUT "https://subman-content.preview.emergentagent.com/api/admin/client-users/CLIENT_EMAIL/status" \
  -H "Content-Type: application/json" \
